...
public sealed class UserServiceTests
{
    ...
    private UserService Service() => new(_repository.Object, new PasswordHasher<User>());

    [Fact]
    public async Task FindByIdAsync_RecursoInexistente_DeveRetornar404()
    {
        // Arrange
        _repository.Setup(r => r.FindByIdAsync(7)).ReturnsAsync((User?)null);
        // Act
        ...
        _repository.Verify(r => r.DeleteAsync(It.IsAny<User>()), Times.Never);
    }
    ...
    public async Task UpdateAsync_RecursoInexistente_DeveRetornar404()
    {
        ...
        _repository.Setup(r => r.FindByIdAsync(7)).ReturnsAsync((User?)null);
        // Act
        var error = await Assert.ThrowsAsync<ApiException>(() => Service().UpdateAsync(7, UserFixture.Request()));
        // Assert
        Assert.Equal(404, error.StatusCode);
        _repository.Verify(r => r.UpdateAsync(It.IsAny<User>()), Times.Never);
        _repository.Verify(r => r.DeleteAsync(It.IsAny<User>()), Times.Never);
    ...
    {
        // Arrange
        var dto = UserFixture.Request();
        User? saved = null;
        _repository.Setup(r => r.CreateAsync(It.IsAny<User>())).ReturnsAsync((User value) => { saved = value; value.Id = 23; return value; });
        // Act
        ...
        Assert.Equal(PasswordVerificationResult.Success, new PasswordHasher<User>().VerifyHashedPassword(saved, saved.Password, dto.Password));
        _repository.Verify(r => r.CreateAsync(It.IsAny<User>()), Times.Once);
    }
    ...
    public async Task UpdateAsync_RecursoExistente_DevePersistirNovosDados()
    {
        ...
        Assert.NotEqual(dto.Password, entity.Password);
        Assert.Equal(PasswordVerificationResult.Success, new PasswordHasher<User>().VerifyHashedPassword(entity, entity.Password, dto.Password));
    ...
    public async Task CreateAsync_EmailDuplicado_DeveRecusarSemPersistir()
    {
        ...
        _repository.Verify(r => r.CreateAsync(It.IsAny<User>()), Times.Never);
    }

    [Fact]
    public async Task CreateAsync_CpfDuplicado_DeveRecusarSemPersistir()
    {
        // Arrange
        var dto = UserFixture.Request();
        _repository.Setup(r => r.ExistsByCpfAsync(dto.Cpf)).ReturnsAsync(true);
        // Act
        var error = await Assert.ThrowsAsync<ApiException>(() => Service().CreateAsync(dto));
        // Assert
        Assert.Equal(400, error.StatusCode);
